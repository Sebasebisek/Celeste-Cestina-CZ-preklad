# Celeste – Český překlad

Tento mod přidává kompletní český překlad hry **Celeste**.  

---

## 📥 Instalace

### 1️⃣ Stáhni a nainstaluj Olympus
Olympus je oficiální mod loader pro Celeste. 
1. Klikni na tlacitko nize pro instalaci
👉 [Stáhnout Olympus](https://github.com/EverestAPI/Olympus/releases/download/v21.01.11.07/Olympus.exe)  
 
---

### 2️⃣ Spusť Olympus a nainstaluj nejnovější Everest
1. Otevři **Olympus**.  
2. Klikni na **Install Everest**.  
3. Klikni nahore na zelenou pripnutou **(stable)** verzi a dole klikni na **Install** (nainstaluje nejnovější verzi).  

🖼️ *Screenshot zde (ukáže tlačítko Install / Update)*  

---

### 3️⃣ Najdi a nainstaluj český překlad
1. V Olympusu klikni na **Download Mods**.  
2. Do vyhledávání napiš **Czech**.  
3. u **Český překlad/Czech translation** Klikni na **Zelene tlacitko**.   

🖼️ *Screenshot zde (ukáže tvůj mod v seznamu Olympus)*  

---

## 🖼️ Ukázky
*(sem vložíš obrázky ze hry – složka `images/`)*  
![Ukázka 1](images/screenshot1.png)  
![Ukázka 2](images/screenshot2.png)  

---
